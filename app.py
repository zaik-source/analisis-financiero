...
.terminal-header::before {
    ...
    background: repeating-linear-gradient(
        ...
        rgba(0,180,216,0.02) 2px, rgba(0,180,216,0.02) 4px
    );
}
.terminal-header h1 {
    font-family: 'IBM Plex Mono', monospace;
    font-size: 1.8rem; font-weight: 700; color: #00b4d8;
...
.terminal-header p {
    ...
    margin: 0.3rem 0 0 0; letter-spacing: 1px;
}
...
.metric-value { font-family: 'IBM Plex Mono', monospace; font-size: 1.4rem; font-weight: 700; color: #e2e8f0; }
.metric-value.positive { color: #00e676; }
.metric-value.negative { color: #ff5252; }
.metric-value.neutral  { color: #ffd740; }
.metric-sub { font-family: 'IBM Plex Mono', monospace; font-size: 0.7rem; color: #64748b; margin-top: 0.2rem; }
...
div[data-testid="stMetric"] {
    background: #0d1421; border: 1px solid #1e3a5f; border-radius: 6px; padding: 1rem;
}
.stButton > button {
    background: linear-gradient(135deg, #00b4d8, #0077b6);
    color: white; border: none; border-radius: 4px;
    font-family: 'IBM Plex Mono', monospace; font-weight: 600;
    letter-spacing: 1px; padding: 0.6rem 2rem; transition: all 0.2s; width: 100%;
}
.stButton > button:hover {
    background: linear-gradient(135deg, #00c4e8, #0087c6);
    box-shadow: 0 0 20px rgba(0,180,216,0.4); transform: translateY(-1px);
}
...
.stTextInput > div > div > input:focus {
    border-color: #00b4d8; box-shadow: 0 0 10px rgba(0,180,216,0.2);
}
.stTabs [data-baseweb="tab"] {
    font-family: 'IBM Plex Mono', monospace; font-size: 0.75rem; letter-spacing: 1px; color: #4a6fa5;
}
.stTabs [aria-selected="true"] { color: #00b4d8 !important; border-bottom: 2px solid #00b4d8 !important; }

.score-bar-bg { background: #1e3a5f; border-radius: 3px; height: 8px; margin-top: 4px; overflow: hidden; }
.score-bar-fill { height: 100%; border-radius: 3px; background: linear-gradient(90deg, #0077b6, #00b4d8); transition: width 0.5s ease; }
.score-row { display: flex; align-items: center; padding: 0.5rem 0; border-bottom: 1px solid #1a2a3a; }
.score-name { font-family: 'IBM Plex Mono', monospace; font-size: 0.72rem; color: #8eacc8; width: 130px; flex-shrink: 0; }
.score-num { font-family: 'IBM Plex Mono', monospace; font-size: 0.8rem; font-weight: 700; color: #00b4d8; width: 50px; text-align: right; flex-shrink: 0; margin-right: 10px; }
...
.warn-box {
    background: rgba(255,215,64,0.08); border: 1px solid #ffd740;
...
def _flatten_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Aplanar MultiIndex de columnas si existe (yfinance lo genera con multi-ticker)."""
...
def cargar_df(ticker: str, meses: int = 24):
    """
    Descarga historial de precios con reintentos.
    Retorna DataFrame vacío si no hay datos; nunca lanza excepción al caller.
    """
    import time
        ...
        try:
            ...
            for col in ('Open', 'High', 'Low', 'Close', 'Volume'):
                if col not in df.columns:
                    raise ValueError(f"Columna '{col}' ausente en la respuesta de yfinance")
    ...
    st.warning(f"⚠️ No se pudieron descargar datos para **{ticker}**: {ultimo_error}")
    return pd.DataFrame()


@st.cache_data(ttl=600, show_spinner=False)
def cargar_info(ticker: str) -> dict:
    """
    Obtiene metadatos del ticker con reintentos.
    Retorna dict vacío en caso de fallo.
    """
    import time
    for intento in range(4):
        try:
...
def calcular_indicadores(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula SMA, Bollinger Bands, RSI y MACD.
    Usa min_periods=1 en rolling para evitar NaN en series cortas.
    """
    df = df.copy()

    # SMAs — min_periods permite operar con <N días de historia
...
def calcular_niveles(df: pd.DataFrame) -> list:
    """
    Detecta soportes y resistencias por swing highs/lows.
    BUG FIX: usaba `df['Low'].iloc` como objeto en lugar de `df['Low'].iloc[i]`.
    """
    niveles = []
    n       = len(df)
    # Mínimo 5 velas para tener índices i-2..i+2
    start   = max(2, n - 180)
    end     = n - 2

    lo = df['Low'].values   # acceso por array numpy: más rápido y sin bug iloc
    hi = df['High'].values

    for i in range(start, end):
        if (lo[i] < lo[i-1] and lo[i] < lo[i+1]
                and lo[i] < lo[i-2] and lo[i] < lo[i+2]):
            niveles.append((float(lo[i]), 'Soporte'))
        elif (hi[i] > hi[i-1] and hi[i] > hi[i+1]
                and hi[i] > hi[i-2] and hi[i] > hi[i+2]):
            niveles.append((float(hi[i]), 'Resistencia'))

    # Agrupar niveles muy cercanos
    limpios  = []
    if niveles:
        niveles.sort()
...
def _safe_float(val, default=0.0) -> float:
    """Convierte a float con fallback; maneja None, NaN e infinitos."""
    try:
        f = float(val)
...
def calcular_scores(info: dict) -> dict:
    """
    BUG FIX: el original usaba `or 0` que convertía 0.0 legítimo a 0.
    Ahora usa _safe_float con defaults explícitos.
    """
    ...
    pe = pe if pe > 0 else 25.0   # evitar división por cero
...
def obtener_sentimiento(ticker: str):
    """
    BUG FIX: ahora reporta explícitamente si la API falló o retornó error.
    Retorna (score, count, titulos, error_msg).
    """
    try:
        url = (
        ...
        if status != 'ok':
            return 0, 0, [], f"NewsAPI error: {res.get('message', status)}"

        arts = res.get('articles', [])
        if not arts:
            return 0, 0, [], "Sin artículos recientes para este ticker"
        ...
        return 0, 0, [], "Tiempo de espera agotado al conectar con NewsAPI"
    except Exception as e:
        return 0, 0, [], f"Error inesperado en NewsAPI: {e}"


@st.cache_data(ttl=600, show_spinner=False)
def cargar_fundamentales(ticker_symbol: str):
    """
    BUG FIX: manejo robusto de columnas faltantes con log detallado.
    Ahora intenta EPS y Shares con múltiples nombres alternativos de yfinance.
    """
    try:
        ticker     = yf.Ticker(ticker_symbol)
        financials = ticker.financials   # columnas = fechas, index = cuentas

        if financials is None or financials.empty:
            return None, "Yahoo Finance no devolvió estados financieros"
        ...
        # EPS: varios nombres posibles según versión de yfinance
        ...
        if not eps_found:
            return None, f"No se encontró EPS. Buscados: {eps_keys}"
        df_f["EPS"] = financials.loc[eps_found]

        shares_found = next((k for k in shares_keys if k in financials.index), None)
        if not shares_found:
            return None, f"No se encontraron acciones. Buscadas: {shares_keys}"
        ...
        if df_f.empty:
            return None, "DataFrame vacío tras dropna()"
        ...
        if hist.empty:
            return None, "Sin historial de precios para calcular múltiplos"

        hist["Year"] = hist.index.year
        precios      = hist.groupby("Year")["Close"].mean()
        dm           = df_f.join(precios, how="inner")

        # Evitar EPS=0 en división
        dm = dm[dm["EPS"] != 0].copy()
        ...
        return None, f"Excepción en fundamentales: {e}"
...
@st.cache_data(ttl=600, show_spinner=False)
def cargar_fundamentales_cached(ticker_symbol: str):
    return cargar_fundamentales(ticker_symbol)
...
def plot_matriz(dm, ticker_symbol):
    ...
    axes[1, 3].legend(frameon=False, fontsize=8, loc="upper left", labelcolor=DARK["text"])
...
with st.sidebar:
    ...
    <div style="text-align:center; padding: 1rem 0 1.5rem 0;">
        <div style="font-family:'IBM Plex Mono',monospace; font-size:1.3rem;
                    font-weight:700; color:#00b4d8; letter-spacing:3px;">TERMINAL</div>
        <div style="font-family:'IBM Plex Mono',monospace; font-size:0.6rem;
                    color:#4a6fa5; letter-spacing:4px; margin-top:2px;">ANÁLISIS INTEGRAL</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    ticker_input = st.text_input(
        ...
        help="Símbolo de Yahoo Finance (ej. AAPL, MSFT, AMXL.MX)"
    ).upper().strip()
...
if not analizar:
    ...
    <div style="text-align:center; padding: 4rem 2rem; font-family:'IBM Plex Mono',monospace;">
        ...
        <div style="font-size:0.65rem; color:#2a4a6a; margin-top:0.5rem;">
            Ejemplos: AAPL · MSFT · TSLA · NVDA · AMXL.MX · WALMEX.MX
        </div>
    </div>
    """, unsafe_allow_html=True)
    st.stop()
...
<div style="display:flex; align-items:baseline; gap:1rem; margin-bottom:1rem; flex-wrap:wrap;">
    ...
    <span style="font-family:'IBM Plex Sans',sans-serif; font-size:1rem; color:#64748b;">{nombre}</span>
...
with tab1:
    ...
    with c2:
        rsi_label = 'Sobrecompra' if rsi_val > 70 else ('Sobreventa' if rsi_val < 30 else 'Zona Neutra')
        st.markdown(f"""
        <div class="metric-card">
    ...
    with st.spinner("Renderizando gráfico técnico..."):
        try:
            ...
            st.error(f"Error renderizando gráfico técnico: {e}")

    # Niveles
    if niveles:
        st.markdown('<div class="section-header">SOPORTES &amp; RESISTENCIAS DETECTADOS</div>',
                    unsafe_allow_html=True)
        ...
        st.markdown('<div class="warn-box">⚠ No se detectaron niveles S/R en el período analizado</div>',
                    unsafe_allow_html=True)
    ...
    with bcol1:
        st.markdown(f"""<div class="metric-card"><div class="metric-label">Banda Superior</div>
        <div class="metric-value" style="font-size:1.1rem;">${bb_upper:.2f}</div></div>""",
                    unsafe_allow_html=True)
    with bcol2:
        st.markdown(f"""<div class="metric-card"><div class="metric-label">SMA20 / Media</div>
        <div class="metric-value" style="font-size:1.1rem;">${bb_sma:.2f}</div>
        <div class="metric-sub">{bb_status}</div></div>""", unsafe_allow_html=True)
    with bcol3:
        st.markdown(f"""<div class="metric-card"><div class="metric-label">Banda Inferior</div>
        <div class="metric-value" style="font-size:1.1rem;">${bb_lower:.2f}</div></div>""",
                    unsafe_allow_html=True)
...
with tab2:
    ...
    with col_scores:
        ...
        st.markdown('<div class="section-header" style="margin-top:1.5rem;">DATOS EMPRESA</div>',
                    unsafe_allow_html=True)
    ...
    # Matriz fundamental
    st.markdown('<div class="section-header">MATRIZ DE VALORACIÓN HISTÓRICA</div>',
                unsafe_allow_html=True)
    with st.spinner("Cargando datos financieros históricos..."):
        dm, fund_error = cargar_fundamentales(ticker_input)
    ...
    if dm is not None and not dm.empty:
        try:
            ...
            st.error(f"Error graficando matriz: {e}")
    else:
        st.markdown(
            f'<div class="warn-box">⚠ Datos fundamentales no disponibles — {fund_error}</div>',
            unsafe_allow_html=True
        )
...
# ═══════════════════════════
with tab3:
    st.markdown('<div class="section-header">ANÁLISIS DE SENTIMIENTO DE NOTICIAS</div>',
                unsafe_allow_html=True)

    with st.spinner("Analizando noticias recientes..."):
        score_sent, count_news, titulos, sent_error = obtener_sentimiento(ticker_input)

    if count_news == 0:
        msg = sent_error or "No se encontraron noticias recientes"
